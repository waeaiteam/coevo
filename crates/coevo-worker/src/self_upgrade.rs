...
use coevo_store::repos_opc::{skill_evolution_repo, agent_memory_repo};

pub struct SelfUpgradeLoop;
impl SelfUpgradeLoop {
    pub async fn run(pool: &SqlitePool, run: &WorkerRun, reflection: &ReflectionRecord, feedback: Option<&str>) -> Result<Option<String>, WorkerError> {
        ...
        if let Ok(Some(mut am)) = agent_memory_repo::AgentMemoryRepo::get(pool, &run.agent_id).await {
            ...
            agent_memory_repo::AgentMemoryRepo::upsert(pool, &am).await.map_err(|e| WorkerError::Internal(e.to_string()))?;
        }

        // Create SkillEvolutionProposal if needed
        let skill_updates: Vec<String> = reflection.skill_to_update_json.as_array().map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect()).unwrap_or_default();
        ...
        if !skill_updates.is_empty() || has_feedback {
            let proposal = SkillEvolutionProposal{
                ...
                diagnosis: if has_feedback { feedback.unwrap_or("").into() } else { format!("Skill update needed: {:?}", skill_updates) },
            ...
            skill_evolution_repo::SkillEvolutionRepo::create_proposal(pool, &proposal).await.map_err(|e| WorkerError::Internal(e.to_string()))?;
