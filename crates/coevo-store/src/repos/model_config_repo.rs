...
impl ModelConfigRepo {
    ...
    fn store_api_key(provider_id: &str, api_key: &str) -> Result<String, sqlx::Error> {
        ...
        let entry =
            keyring_core::Entry::new(CREDENTIAL_SERVICE, &Self::credential_user(provider_id))
                .map_err(|e| {
                    sqlx::Error::Protocol(format!("CREDENTIAL_VAULT_ENTRY_FAILED: {}", e))
                })?;
        entry
    ...
    fn delete_api_key(provider_id: &str) -> Result<(), sqlx::Error> {
        ...
        match entry.delete_credential() {
            ...
            Err(e) => Err(sqlx::Error::Protocol(format!(
                "CREDENTIAL_VAULT_DELETE_FAILED: {}",
                e
            ))),
        }
    }
    ...
    pub async fn set_active(pool: &SqlitePool, provider_id: &str) -> Result<(), sqlx::Error> {
        ...
        sqlx::query("UPDATE model_provider_configs SET is_active=0,updated_at_ms=?")
            .bind(now)
            .execute(pool)
            .await?;
        sqlx::query(
            "UPDATE model_provider_configs SET is_active=1,updated_at_ms=? WHERE provider_id=?",
        )
        .bind(now)
        .bind(provider_id)
        .execute(pool)
    ...
    pub async fn deactivate_others(
        ...
        sqlx::query(
            "UPDATE model_provider_configs SET is_active=0,updated_at_ms=? WHERE provider_id!=?",
        )
    ...
    pub async fn clear_api_key(pool: &SqlitePool, provider_id: &str) -> Result<(), sqlx::Error> {
        ...
        sqlx::query("UPDATE model_provider_configs SET api_key_ciphertext='',api_key_masked='',updated_at_ms=? WHERE provider_id=?")
            .bind(chrono::Utc::now().timestamp_millis()).bind(provider_id).execute(pool).await?;
        Ok(())
    }
}
