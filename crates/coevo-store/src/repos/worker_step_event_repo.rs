...
impl WorkerRunStepRepo {
    pub async fn append(
        ...
        sqlx::query("INSERT INTO worker_run_steps (step_id, session_id, step_type, input_json, output_json, status, created_at_ms) VALUES (?,?,?,?,?,?,?)")
            .bind(step_id).bind(session_id).bind(step_type).bind(input).bind(output).bind(status).bind(created).execute(pool).await?;
        Ok(())
    }
...
impl WorkerEventRepo {
    pub async fn append(
        ...
        sqlx::query("INSERT INTO worker_events (event_id, session_id, event_type, payload_json, created_at_ms) VALUES (?,?,?,?,?)")
            .bind(event_id).bind(session_id).bind(event_type).bind(payload).bind(created).execute(pool).await?;
        Ok(())
    }
    pub async fn list_by_session(
        pool: &SqlitePool,
