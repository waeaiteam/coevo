...
pub fn seed_employees() -> Vec<AgentEmployee> {
    ...
    vec![
        employee("agent-founder-01", "Founder Assistant", Department::FounderOffice, "FounderOffice", vec!["Hypothesis","Suggestion"], vec!["DRAFT_ONLY"], 0.3, now),
        employee("agent-pm-01", "Product Manager", Department::Product, "Product", vec!["Suggestion"], vec!["DRAFT_ONLY"], 0.3, now),
        ...
        employee("agent-synth-01", "Synthesizer", Department::FounderOffice, "FounderOffice", vec!["Suggestion"], vec!["DRAFT_ONLY"], 0.3, now),
    ]
}

fn employee(
    agent_id: &str, name: &str, dept: Department, role: &str,
    layers: Vec<&str>, actions: Vec<&str>, risk_ceiling: f64, now: u64,
) -> AgentEmployee {
    AgentEmployee {
        ...
        passport: AgentPassport {
            ...
            restrictions: vec!["no production write".to_string(), "no financial transfer".to_string()],
            expires_at_ms: None,
        },
        model_profile: ModelProviderProfile {
            provider: "mock".to_string(), base_url: String::new(), api_key_ref: String::new(),
            default_model: "gpt-4o".to_string(), fast_model: "gpt-4o-mini".to_string(),
            reasoning_model: "o1".to_string(), structured_output_model: "gpt-4o".to_string(),
            timeout_ms: 30000, max_tokens: 4096, max_cost_per_task_usd: 1.0,
        },
        tool_scopes: vec!["urn:coevo:tool:read".to_string()],
        memory_scope: MemoryScope::Agent,
        permission_boundary: PermissionBoundary {
            max_risk_score: risk_ceiling, can_write_fact: false, can_write_decision: false,
            can_access_network: false, can_access_filesystem: false,
            can_call_external_executor: false, can_propose_skill: true,
        },
