use crate::enum_db::{
    memory_scope_from_db, memory_scope_to_db, memory_status_from_db, memory_status_to_db,
};
...
impl MemoryRepo {
    fn from_row(row: &sqlx::sqlite::SqliteRow) -> MemoryRecord {
        ...
        MemoryRecord {
            ...
            scope: memory_scope_from_db(&scope_s),
            ...
            status: memory_status_from_db(&status_s),
            cognitive_layer: serde_json::from_str(&format!("\"{}\"", layer_s))
    ...
    pub async fn list(
        ...
        if scope.is_some() {
            q.push_str(" AND scope=?");
        }
        if owner_id.is_some() {
            q.push_str(" AND owner_id=?");
        }
        if !include_revoked {
            q.push_str(" AND status != 'Revoked'");
        }
    ...
    pub async fn mark_stale(pool: &SqlitePool, mid: &str) -> Result<(), sqlx::Error> {
        sqlx::query("UPDATE memory_records SET status='Stale',updated_at_ms=? WHERE memory_id=?")
    ...
    pub async fn revoke(pool: &SqlitePool, mid: &str) -> Result<(), sqlx::Error> {
        sqlx::query("UPDATE memory_records SET status='Revoked',updated_at_ms=? WHERE memory_id=?")
            .bind(chrono::Utc::now().timestamp_millis())
            .bind(mid)
            .execute(pool)
            .await?;
