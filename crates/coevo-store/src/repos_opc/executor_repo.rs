...
impl ExecutorRepo {
    fn from_row(row: &sqlx::sqlite::SqliteRow) -> ExternalExecutorPassport {
        ...
        fn de<T: serde::de::DeserializeOwned>(s: &str) -> T { serde_json::from_str(s).unwrap_or_else(|_| panic!("Deserialize failed")) }
        ExternalExecutorPassport{executor_id:row.get("executor_id"),display_name:row.get("display_name"),
