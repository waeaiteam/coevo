use crate::enum_db::{
    executor_source_type_from_db, executor_source_type_to_db, executor_status_from_db,
    executor_status_to_db, memory_scope_from_db, memory_scope_to_db, sandbox_level_from_db,
    sandbox_level_to_db,
};
use coevo_core::opc::*;
...
impl ExecutorRepo {
    fn from_row(row: &sqlx::sqlite::SqliteRow) -> Result<ExternalExecutorPassport, sqlx::Error> {
        ...
        let sl: String = row.get("sandbox_level");
        let ms: String = row.get("memory_scope");
        let s: String = row.get("status");
        fn de<T: serde::de::DeserializeOwned>(s: &str) -> Result<T, sqlx::Error> {
            serde_json::from_str(s)
                .map_err(|e| sqlx::Error::Protocol(format!("Deserialize failed: {}", e)))
        }
        Ok(ExternalExecutorPassport {
            executor_id: row.get("executor_id"),
            display_name: row.get("display_name"),
            source_type: executor_source_type_from_db(&st),
            runtime_endpoint: row.get("runtime_endpoint"),
            capabilities: de(row.get("capabilities_json"))?,
            required_credentials: de(row.get("required_credentials_json"))?,
            permission_boundary: de(row.get("permission_boundary_json"))?,
            file_scope: de(row.get("file_scope_json"))?,
            network_scope: de(row.get("network_scope_json"))?,
            memory_scope: memory_scope_from_db(&ms),
            risk_ceiling: row.get("risk_ceiling"),
            supported_actions: de(row.get("supported_actions_json"))?,
            sandbox_level: sandbox_level_from_db(&sl),
            health_check_url: row.get("health_check_url"),
            audit_callback_url: row.get("audit_callback_url"),
            status: executor_status_from_db(&s),
    ...
    pub async fn list(pool: &SqlitePool) -> Result<Vec<ExternalExecutorPassport>, sqlx::Error> {
        let rows = sqlx::query("SELECT * FROM external_executors ORDER BY created_at_ms DESC")
            .fetch_all(pool)
            .await?;
    ...
    pub async fn get(
        pool: &SqlitePool,
        id: &str,
    ) -> Result<Option<ExternalExecutorPassport>, sqlx::Error> {
        let row = sqlx::query("SELECT * FROM external_executors WHERE executor_id=?")
            .bind(id)
            .fetch_optional(pool)
            .await?;
    ...
    pub async fn disable(pool: &SqlitePool, id: &str) -> Result<(), sqlx::Error> {
        sqlx::query(
            "UPDATE external_executors SET status='Disabled',updated_at_ms=? WHERE executor_id=?",
        )
