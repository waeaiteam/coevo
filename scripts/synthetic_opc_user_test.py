#!/usr/bin/env python3
"""coevo-opc Synthetic User Test — Production-Grade Foundation verification."""
...
def t(name, fn):
    ...
    try:
        ...
        print(f"  PASS {name}")
        PASS += 1
    except AssertionError as e: print(f"  FAIL {name}: {e}"); FAIL += 1; ERRORS.append(name)
    except Exception as e: print(f"  FAIL {name}: ERROR {e}"); FAIL += 1; ERRORS.append(name)
...
def req(method, path, body=None):
    ...
    try:
        ...
        except: return {"_status": e.code, "_error": str(e)}
...
t("4. PUT /opc/models/config kind=Mock", lambda: put("/opc/models/config", {"provider_id":"test-mock","kind":"Mock"})["ok"] == True)
...
# Red WorkOrder
red_wo = post("/opc/work-orders", {"contract_hash":CH,"plan_hash":PH,"user_id":"df","opc_id":"do","mission_intent":"Delete production DB","selected_agents":[],"selected_executors":[],"required_skills":[],"track":"red","allowed_actions":[],"restricted_actions":["delete"],"risk_summary":"high"})
...
t("29. Red execute without creds → 403", lambda: red_exec.get("_status") == 403)
