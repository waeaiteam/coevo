  ...
  async function createFoundation() {
    ...
    try {
      ...
      await createMemory({
        ...
        source: "first-run",
        provenance: `first-run:${current.opcId}:company-foundation`,
        confidence: 0.9,
        ttl_seconds: 2592000,
        created_at_ms: now,
        updated_at_ms: now,
        ...
        <section className="rounded-xl border p-5 shadow-sm" style={{ background: "var(--bg-card)", borderColor: "var(--border-subtle)" }}>
          {step === "identity" && (
            <div className="space-y-4">
              ...
              <button onClick={continueToFoundation} className="w-full rounded-md py-3 text-sm font-semibold text-white" style={{ background: "var(--accent)" }}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="rounded-md border p-2" style={{ borderColor: "var(--border-subtle)" }}>
                  ...
                  <div className="font-semibold">{busy && teamCount === null ? t("settings.loading") : `${teamCount ?? 0} ${t("first_run.agents")}`}</div>
                </div>
                <div className="rounded-md border p-2" style={{ borderColor: "var(--border-subtle)" }}>
                  <div style={{ color: "var(--text-muted)" }}>{t("first_run.skills_preview")}</div>
                  <div className="font-semibold">{busy && skillCount === null ? t("settings.loading") : `${skillCount ?? 0} ${t("first_run.skills")}`}</div>
                </div>
              </div>
              <div>
          ...
          {step === "handoff" && (
            <div className="space-y-4">
              ...
              <button onClick={openModelProvider} className="w-full rounded-md py-3 text-sm font-semibold text-white" style={{ background: "var(--accent)" }}>
