...
pub async fn list_workers(State(s): State<AppState>) -> (StatusCode, Json<serde_json::Value>) {
    AgentWorkerRepo::list(&s.pool).await.map_or_else(
...
pub async fn get_worker_runs(
    ...
    WorkerRunRepo::list_by_work_order(&s.pool, &id)
        .await
        .map_or_else(
            |e| err!(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()),
            |rows| ok!(serde_json::json!(rows_to_json(rows))),
...
pub async fn get_run_steps(
    ...
    WorkerStepRepo::list_by_run(&s.pool, &run_id)
        .await
        .map_or_else(
            |e| err!(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()),
            |rows| ok!(serde_json::json!(rows_to_json(rows))),
...
pub async fn get_run_events(
    ...
    WorkerEventRepo::list_by_run(&s.pool, &run_id)
        .await
        .map_or_else(
            |e| err!(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()),
            |rows| ok!(serde_json::json!(rows_to_json(rows))),
