...
async fn test_05_red_missing_dual_sign_rejected() {
    ...
    // Red Track with valid identity but NO monitoring AND NO diagnostic → lease denied
    ...
    match result {
        Ok(r) => {
            // If it succeeded, lease MUST be None (no dual-sign → no lease)
            assert!(r.lease.is_none(),
                "Without both monitoring and diagnostic signatures, lease must NOT be granted");
        }
        Err(e) => {
            // It's OK if it fails for other reasons (e.g., circuit breaker)
            // But must NOT be MissingIdentityProof
            assert!(!e.to_string().contains("caller_identity_proof"),
                "Must not reject with identity error when identity is provided: {}", e);
        }
    }
}
...
async fn test_10_red_with_identity_generates_lease() {
    ...
    match result {
        Ok(r) => {
            ...
            // If lease was granted, verify it
            ...
            // Even without lease (if gating denied), must have a valid decision
            assert!(!r.decision.is_empty(), "decision must not be empty");
        }
        Err(e) => {
            // Must NOT be MissingIdentityProof
            assert!(
                !e.to_string().contains("caller_identity_proof"),
                "With valid identity_proof, must not fail with MissingIdentityProof. Error: {}",
                e
            );
        }
    }
